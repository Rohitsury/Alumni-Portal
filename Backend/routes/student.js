...
const UserOTPVerification = require('../model/studentmodel/UserOtpVerificationSchema');
const Student = require('../model/studentmodel/RegisterSchema');
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      ...
      const newStudent = new Student({ name, email, phone,  password: secpassword, verified: false });
...
router.post('/verifyotp', async (req, res) => {
  try {
    let { id:userId, otp,name, email, phone   } = req.body;
    console.log(userId, otp, name, email, phone)
    if (!userId || !otp) {
      ...
      if (!userOTPVerificationRecord) {
        ...
        if (expiresAt < Date.now()) {
          ...
          if (!validOTP) {
            ...
            await Student.updateOne({ _id: userId }, { verified: true, name, email, phone });
            await UserOTPVerification.deleteOne({ userId });
            res.status(200).json({
              status: "VERIFIED",
...
router.post('/createprofile', authenticateUser, async (req, res) => {
  const { name, email, phone, address, dob,  branch, sem, cgpa, skills, hobbies, languagesknown, projects, profileimg, resume } = req.body;
